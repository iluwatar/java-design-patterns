...
import com.iluwatar.choreography.events.*;
...
public class MainService {
    ...
    /**
     * Generates new sagaId values, through which we can track the flow of events through multiple microservices
     */
    private final AtomicInteger sagaCounter = new AtomicInteger();

    /**
     * The method call that kicks off the whole flow.
     * It creates a sagaId, and submits the first event
     *
     ...
     * The message queue
     * <p>
     * Events get fed into here, and other services are notified of the event
     * The other services are able to submit events back to the queue as well
     *
    ...
    public Response post(Event event) {
        if (event == null) {
            ...
            if (event instanceof RequestScheduleDeliveryEvent) {
                deferredResponse = CompletableFuture.supplyAsync(() -> packageService.getPackage((RequestScheduleDeliveryEvent) event));
            } else if (event instanceof PackageEvent) {
                deferredResponse = CompletableFuture.supplyAsync(() -> droneService.getDrone((PackageEvent) event));
            } else if (event instanceof DroneEvent) {
                deferredResponse = CompletableFuture.supplyAsync(() -> deliveryService.completeDelivery((DroneEvent) event));
            } else if (event instanceof DeliverySuccessEvent) {
                deferredResponse = CompletableFuture.supplyAsync(() -> new OK((DeliverySuccessEvent) event));
                ...
                deferredResponse = CompletableFuture.supplyAsync(() -> new Failure(event.prettyPrintSagaId() + "Could not handle that type of event!"));
            }
