...
/**
 * The Bulkhead pattern isolates critical system resources for each service or
 * component to prevent failures or heavy load in one part from cascading and
 * degrading the entire system.
 *
 * <p>
 * In this example, we demonstrate resource isolation using separate thread
 * pools for different services. The {@link UserService} handles user-facing
 * requests while {@link BackgroundService} handles background tasks. Each
 * service has its own dedicated thread pool (bulkhead), ensuring that if one
 * service becomes overloaded, the other continues to function normally.
 *
 * <p>
...
public class App {
    ...
    /**
     * Demonstrates normal operation where both services handle requests
     * successfully.
     *
    ...
    /**
     * Demonstrates overloading the background service while user service
     * remains operational. This shows how the bulkhead pattern prevents cascade
     * failures.
     *
     * @param userService the user-facing service
     * @param backgroundService the background processing service
    ...
    private static void demonstrateServiceOverload(
        ...
        for (int i = 1; i <= 3; i++) {
            boolean accepted
                    = userService.submitTask(new Task("Critical-User-Request-" + i, TaskType.USER_REQUEST, 300));
            LOGGER.info("User request {} accepted: {}", i, accepted);
        }
    }

    /**
     * Demonstrates resource isolation by showing independent operation of
     * services.
     *
     * @param userService the user-facing service
     * @param backgroundService the background processing service
